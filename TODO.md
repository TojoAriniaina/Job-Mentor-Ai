...
- [x] **Anomalies de `LettreController::analyze()` trop généreuses** — les détecteurs locaux redondants (« formule d'ouverture répétée », « segment dupliqué », « ligne dupliquée », clôture, caractères/espaces parasites) ont été supprimés de `analyze()` : la vérité appartient à `AtsScorer::calculateLetterQualityScore()`, dont les anomalies (règles ancorées + listes d'exclusion) sont déjà fusionnées dans le diagnostic. Une lettre correcte avec « Madame, Monsieur » en ouverture + formule de politesse finale ne remonte plus aucune anomalie ; une vraie duplication (« À l'attention du À l'attention du… ») est toujours signalée.
- [x] **Jeton CSRF sur les routes en session** — `src/Middleware/Csrf.php` : jeton par session (`$_SESSION['csrf_token']`) vérifié par le front controller avant dispatch sur tout POST/DELETE (403 JSON sinon), précédé d'un contrôle d'origine (Origin/Referer) qui protège aussi login/register/request-reset (exemptés de jeton, faute de session préalable). Jeton servi par `/api/auth/check` (même invité) + cookie `csrf_token` lisible par JS posé sur chaque réponse API ; injection automatique via un wrapper `window.fetch` dans `config.js` (en-tête `X-CSRF-Token` sur les appels same-origin vers `/api/` uniquement — jamais vers les CDN). Rotation du jeton à login/register.
- [x] **Bibliothèques CDN à héberger en local** — pdf.js 3.11.174, tesseract.js 5.1.1 (+ core wasm, données `fra`/`eng`), pdfmake 0.2.7, jsPDF 2.5.1, html2canvas 1.4.1, Font Awesome 6.5.0 et les polices Google (Inter, Outfit, JetBrains Mono) sont désormais servis depuis `public/frontend/libs/`. Vérifié headless hors CDN : zéro requête externe sur toutes les pages, import OCR d'un PDF scanné et exports PDF fonctionnels.
- [x] **Quota ElevenLabs** — `TtsController` mappe désormais les erreurs : quota dépassé / 429 → HTTP 503 + message français explicite (`code: tts_quota`), clé invalide / 401 → idem ; `ai.js` affiche le message du backend via un toast et la voix du navigateur prend le relais.
- [x] **Vérifier les règles `.htaccess` sous Apache** — contrôlé sur une instance Apache temporaire (docroot = racine du projet, port de test, arrêtée après contrôle) : `.env`, `.env.example`, `AGENTS.md`, `database.sql`, `logs/*.log`, `composer.json`/`.lock`, `src/`, `config/`, `bootstrap/`, `vendor/`, `tools/` répondent tous **403** ; `/public/index.php` et les assets 200, `/api/auth/check` 200 en JSON, route inconnue 404 ; en-têtes `nosniff`, `X-Frame-Options`, `Referrer-Policy` présents. À re-contrôler sur le serveur réel après tout changement de ces règles.
